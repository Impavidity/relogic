...
wget -O "${SRLPATH}/srlconll-1.1.tgz" https://www.cs.upc.edu/~srlconll/srlconll-1.1.tgz
...
do
  echo $s
  cat ${WSJPATH}/$s/* | wsj-removetraces.pl | wsj-to-se.pl -w 1 | awk '{print $1}' | \
...
cd ${CONLL05_PATH}
tcsh ./scripts/make-trainset.sh
tcsh ./scripts/make-devset.sh
