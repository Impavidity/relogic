...
class RelExtractionExample(object):
  ...
  def process(self, tokenizer, extra_args=None):
    ...
    self.end_of_subject = self.head_index[self.subj_span[1]]
    ...
    self.end_of_object = self.head_index[self.obj_span[1]]
...
class RelExtractionInputFeature(object):
  def __init__(self, input_ids, input_mask,
    ...
    self.end_of_subject = end_of_subject
    self.start_of_object = start_of_object
    self.end_of_object = end_of_object
  ...
  for idx, example in enumerate(examples):
    ...
    features.append(
      RelExtractionInputFeature(
        ...
        end_of_subject=example.end_of_subject,
        end_of_object=example.end_of_object))
...
def generate_rel_extraction_input(mb, config, device, use_label):
  ...
  extra_args = {}
  subj_indicator = create_tensor(mb.input_features, "subj_indicator", torch.long, device)
  obj_indicator = create_tensor(mb.input_features, "obj_indicator", torch.long, device)
  extra_args['subj_indicator'] = subj_indicator
  extra_args['obj_indicator'] = obj_indicator
  extra_args['start_of_subject'] = create_tensor(mb.input_features, "start_of_subject", torch.long, device)
  extra_args['start_of_object'] = create_tensor(mb.input_features, "start_of_object", torch.long, device)
  extra_args["end_of_subject"] = create_tensor(mb.input_features, "end_of_subject", torch.long, device)
  extra_args["end_of_object"] = create_tensor(mb.input_features, "end_of_object", torch.long, device)
